...
├── cli/                 # Typer CLI (doctor, version commands)
...
├── manifest/            # metadata.json, plugin.template.json, marketplace.template.json
└── tests/               # Plugin smoke tests

dist/plugins/superagent/ # Built plugin artifacts (generated by make build-plugin)
└── .claude-plugin/      # Manifests: plugin.json, marketplace.json
    └── tests/           # Plugin tests

# Legacy directories (being phased out)
setup/                   # Old component structure
superclaude/             # Legacy CLI (pre-rebrand)
...
**ABI-first design**: Python modules under `src/superagent/api/` provide a reusable ABI (Application Binary Interface) so MCP gateways, CLIs, and IDEs can share the same workflow logic without duplicating slash-command specs.
...
**Branch structure**: `main` (production) ← `next` (testing) ← `feature/*`, `fix/*`, `docs/*`
...
4. Merge to `next` → validate → merge to `main`
...
git worktree add ../superagent-next next
...
**Usage**:
- Session A: Open `~/github/superagent/` (current branch: next)
- Session B: Open `~/github/superagent-next/` (next)
- Session C: Open `~/github/superagent-feature/` (feature branch)

**Cleanup**:
```bash
git worktree remove ../superagent-next
```
...
**Output**: `dist/plugins/superagent/.claude-plugin/` contains rendered manifests.

### Editing Workflow

1. Update metadata: `plugins/superagent/manifest/metadata.json`
...
**Entry points**:
- CLI: `superagent` command (via `superagent.cli.main:main`)
- Pytest plugin: Auto-loaded as `superagent` (via `superagent.pytest_plugin`)

**Dependencies**:
- pytest>=7.0.0 (testing framework)
- click>=8.0.0 (CLI framework - note: being migrated to typer)
