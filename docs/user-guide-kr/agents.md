# Airis Agent 에이전트 가이드 🤖

Airis Agent는 Claude Code가 전문 지식을 위해 호출할 수 있는 15개의 도메인 전문 에이전트를 제공합니다.
...
#### 2. 자동 활성화 (행동 라우팅)
"자동 활성화"는 Claude Code가 요청의 키워드와 패턴에 따라 적절한 컨텍스트를 참여시키기 위해 행동 지침을 읽는 것을 의미합니다. Airis Agent는 Claude가 가장 적절한 전문가에게 라우팅하기 위해 따르는 행동 가이드라인을 제공합니다.
...
**전문가 지원:**
- `Airis Agent install --diagnose` 사용
- 조정 분석은 [진단 참조 가이드](../reference/diagnostic-reference.md) 참조

**커뮤니티 지원:**
- [GitHub Issues](https://github.com/agiletec-inc/airis-agent/issues)에서 문제 보고
...
### 필수 문서
- **[명령어 가이드](commands.md)** - 최적의 에이전트 조정을 트리거하는 Airis Agent 명령어 마스터
...
### 개발 리소스
- **[기술 아키텍처](../developer-guide/technical-architecture.md)** - Airis Agent의 에이전트 시스템 설계 이해
