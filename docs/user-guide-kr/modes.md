...
- **프레임워크 규정 준수**: Airis Agent 원칙 및 품질 게이트에 대한 행동 검증
...
| **M003** | 모드를 인식할 수 없음 | Airis Agent 업데이트 또는 철자 확인 |
...
**수준 4: 커뮤니티 지원**
- [GitHub Issues](https://github.com/agiletec-inc/airis-agent/issues)에서 모드 문제 보고
- 예상치 못한 모드 동작 예제 포함
...
Airis Agent의 5가지 행동 모드는 필요에 따라 자동으로 일치하는 **지능형 적응 시스템**을 만듭니다:
...
**핵심 인사이트**: 모드에 대해 생각할 필요가 없습니다 - 개발 경험을 향상시키기 위해 투명하게 작동합니다. 달성하고자 하는 것을 설명하면 Airis Agent가 자동으로 필요에 맞게 접근 방식을 조정합니다.
