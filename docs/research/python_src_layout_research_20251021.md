...
**Date**: 2025-10-21
**Question**: Should `superagent` repository use `src/airis_agent/` (nested) or simpler structure?
...
**結論**: `src/airis_agent/` の二重ネストは**正しい**が、**必須ではない**
...
**選択肢**：
1. **標準的** (PyPA推奨): `src/airis_agent/` ← 今の構造
...
**あなたの疑問**:
> リポジトリ名が `superagent` なのに、なぜ `src/airis_agent/` と重複？

**答え**:
1. **リポジトリ名** (`superagent`): GitHub上の名前、プロジェクト全体
2. **パッケージ名** (`src/airis_agent/`): Pythonで `import airis_agent` する際の名前
...
**研究完了**: 2025-10-21
**信頼度**: High (90%)
**推奨**: **変更不要** - 現在の `src/airis_agent/` 構造は最新ベストプラクティス
