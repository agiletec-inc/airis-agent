...
**Date**: 2025-10-21
**Question**: Should `superagent` repository use `src/superagent/` (nested) or simpler structure?
...
**結論**: `src/superagent/` の二重ネストは**正しい**が、**必須ではない**
...
**選択肢**：
1. **標準的** (PyPA推奨): `src/superagent/` ← 今の構造
2. **シンプル** (可能): `src/` のみでモジュール直下に配置
3. **フラット** (古い): リポジトリ直下に `superagent/`
...
**あなたの疑問**:
> リポジトリ名が `superagent` なのに、なぜ `src/superagent/` と重複？

**答え**:
1. **リポジトリ名** (`superagent`): GitHub上の名前、プロジェクト全体
2. **パッケージ名** (`src/superagent/`): Pythonで `import superagent` する際の名前
...
**デメリット**:
- ❌ パス が長い: `src/superagent/pm_agent/confidence.py`
- ❌ 一見冗長に見える
...
**研究完了**: 2025-10-21
**信頼度**: High (90%)
**推奨**: **変更不要** - 現在の `src/superagent/` 構造は最新ベストプラクティス
