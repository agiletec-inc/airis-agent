...
- **模式识别**：识别重复行为和改进机会
- **元学习**：提取见解以持续改进框架
- **框架合规性**：根据 Airis Agent 原则和质量门控验证行为
...
| **M003** | 模式无法识别 | 更新 Airis Agent 或检查拼写 |
...
**级别 4：社区支持**
- 在 [GitHub Issues](https://github.com/agiletec-inc/airis-agent/issues) 报告模式问题
- 包括意外模式行为的示例
- 描述期望与实际模式激活的差异
...
Airis Agent 的 5 种行为模式创建了一个**智能适应系统**，自动匹配您的需求：
...
**关键洞察**：您无需思考模式 - 它们透明地工作以增强您的开发体验。只需描述您想要完成的任务，Airis Agent 会自动调整其方法以匹配您的需求。
