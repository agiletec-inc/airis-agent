...
- **cleanup**  
  ...
  - 標準 `/cleanup` より「安全性評価・ペルソナ連携」が差別化要素。→ **Airis Agent 版として再収録推奨**。  
- **explain**  
...
=> 上記 8 コマンドは「名称の偶然一致はあるが、Airis Agent 仕様として明確に強化された振る舞い」を持つ。  
...
| `cleanup` | Claude 標準名 + 拡張版 | Airis Agent の実装後フェーズに統合 | self-review 後に必要なら自動で走らせる。 |
| `design` | Airis Agent オリジナル | Airis Agent 内の設計モードとして保持 | 明示コマンドも残し、設計特化タスクで案内。 |
| `document` | Airis Agent オリジナル | 自動ドキュメント更新ワークフローに組み込み | 成果報告時にトリガー。 |
